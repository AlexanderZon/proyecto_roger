				  ...
				  <div class="modal-dialog">
				    ...
				    <div class="modal-content">
				      <div class="modal-header">
				        ...
				        <h4 class="modal-title" id="modalFormularioAnadirLabel">Nuevo Grado</h4>
				      </div>
				...
				<div class="modal fade" id="modalFormularioEditar" tabindex="-1" role="dialog" aria-labelledby="modalFormularioEditarLabel" aria-hidden="true">
				  <div class="modal-dialog">
				    ...
				    <div class="modal-content">
				      <div class="modal-header">
				        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				        <h4 class="modal-title" id="modalFormularioAnadirLabel">Editar Grado</h4>
				      </div>
				      <div class="modal-body">
				          <div class="form-group">
				            <label for="siglas" class="control-label">Siglas</label>
				...
				<div class="modal fade" id="modalFormularioEliminar" tabindex="-1" role="dialog" aria-labelledby="modalFormularioEliminarLabel" aria-hidden="true">
				  <div class="modal-dialog">
				    ...
				    <div class="modal-content">
				      <div class="modal-header">
				        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				        <h4 class="modal-title" id="modalFormularioAnadirLabel">Eliminar Grado</h4>
				      </div>
				      <div class="modal-body">
				          <div class="form-group">
				            <label for="siglas" class="control-label">Siglas</label>
				            <input type="text" class="form-control" id="siglas" name="siglas" required readonly>
				          </div>
				          <div class="form-group">
				            <label for="descripcion" class="control-label">Descripcion</label>
			    ...
			    <tbody>
			    	@foreach($grados as $grado)
				        <tr>
				            ...
				            <td>{{$grado->siglas}}</td>
				            <td>
								<button type="button" class="btn btn-warning label label-warning" data-toggle="modal" data-target="#modalFormularioEditar" data-descripcion="{{$grado->descripcion}}" data-siglas="{{$grado->siglas}}" data-action="{{$route}}/editar/{{$grado->id}}">Editar</button> 
								<button type="button" class="btn btn-danger label label-danger" data-toggle="modal" data-target="#modalFormularioEliminar" data-descripcion="{{$grado->descripcion}}" data-siglas="{{$grado->siglas}}" data-action="{{$route}}/borrar/{{$grado->id}}">Eliminar</button> 
							</td>
				        </tr>
			        @endforeach
			    </tbody>
	...
	<script type="text/javascript">
		...
		$(document).ready( function () {
		    ...
		    $('#modalFormularioEditar').on('show.bs.modal', function (event) {
			  ...
			  var siglas = button.data('siglas') // Extract info from data-* attributes
			  var action = button.data('action') // Extract info from data-* attributes
			  if(descripcion != ''){
				  var modal = $(this);
				  $('#formularioEditar').attr('action', action);	
				  modal.find('.modal-title').text('Editar el Grado: ' + descripcion);
				  modal.find('.modal-body input[name=descripcion]').val(descripcion);  	
				  modal.find('.modal-body input[name=siglas]').val(siglas);  
			  }
		    ...
		    $('#modalFormularioEliminar').on('show.bs.modal', function (event) {
			  ...
			  if(descripcion != ''){
				  ...
				  modal.find('.modal-title').text('Eliminar el Grado: ' + descripcion);
				  modal.find('.modal-body input[name=descripcion]').val(descripcion);  	
				  modal.find('.modal-body input[name=siglas]').val(siglas);  	
			  }
			  // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
